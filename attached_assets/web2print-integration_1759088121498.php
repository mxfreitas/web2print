...
class Web2PrintIntegration {
    ...
    public function ajax_upload_pdf() {
        ...
        try {
			...
			if ($uploaded_file === false || !is_array($uploaded_file)) {
    wp_send_json_error(array(
        // Mensagem de erro genérica, pois não temos a mensagem do wp_handle_upload
        'message' => __('Falha desconhecida ao mover arquivo para o diretório de uploads.', 'web2print-integration')
    ));
    return;
}

// Verificação de erro padrão do wp_handle_upload (para arrays com erro)
...
// Agora é seguro acessar $uploaded_file['url']
...
if (isset($flask_analysis['error'])) { // Se a API Flask retornar um indicador de erro
    $error_message = $flask_analysis['message'] ?? 'Erro desconhecido na análise do PDF.';
    wp_send_json_error(array(
        'message' => __('Erro na análise do PDF. Detalhe: ', 'web2print-integration') . $error_message
    ));
    return;
}
